...
**版本**: 3.0  
**日期**: 2026-01-08  
**技術棧**: Flask Blueprint + SQLite + HTML/CSS/JS + RESTful API  
**系統名稱**: Order Tracking System (`order_tracking`)  
**更新內容**: 
- ✅ 精簡Blueprint架構設計
- ✅ 完整RESTful API（為APP準備）
- ✅ JWT認證機制
- ✅ 用戶權限系統
- ✅ 可獨立運行 + 可整合到主應用  
...
9. [快速更新功能](#快速更新功能)
10. [用戶認證系統](#用戶認證系統)
11. [RESTful API設計](#restful-api設計)
12. [APP接入指南](#app接入指南)
13. [開發計劃](#開發計劃)
...
本系統採用**Flask Blueprint**架構，具備以下特性：
- ✅ **可獨立運行** - 開發測試時單獨運行
- ✅ **可整合部署** - 隨時引入到主Flask應用
- ✅ **模塊化設計** - 代碼結構清晰
- ✅ **API優先** - 為移動APP預留完整接口
...
| 項目 | 命名 | 說明 |
|------|------|------|
| **包名** | `order_tracking/` | 訂單流程追蹤系統 |
| **藍圖變量** | `tracking_bp` | Blueprint實例 |
| **URL前綴** | `/tracking` | 所有路由前綴 |
| **CSS文件** | `tracking.css` | 統一樣式表 |
| **JS文件** | `tracking.js` | 統一腳本 |
| **數據庫** | `tracking.db` | SQLite數據庫 |
| **模板路徑** | `templates/tracking/` | 避免衝突 |
...
│
├── app.py                              # 主應用入口
├── order_system.py                     # 舊系統（保留）
├── ... 其他現有文件 ...
│
├── order_tracking/                     # 【新增】訂單追蹤系統
│   │
│   ├── __init__.py                    # 藍圖入口 + 所有路由（300-400行）
│   ├── models.py                      # 數據模型（100-150行）
│   ├── config.py                      # 配置文件（30行）
│   ├── app.py                         # 獨立運行入口（30行）
│   │
│   ├── static/                        # 靜態資源
│   │   ├── tracking.css              # 所有樣式（500-1000行）
│   │   └── tracking.js               # 所有交互邏輯（300-500行）
│   │
│   ├── templates/                     # HTML模板
│   │   └── tracking/                 # 子目錄避免衝突
│   │       ├── base.html             # 基礎模板
...
│   │       ├── order_form.html       # 新增/編輯訂單
│   │       ├── revisions.html        # 修圖列表
│   │       ├── revision_detail.html  # 修圖詳情
│   │       └── reports.html          # 統計報表
│   │
│   └── data/                          # 數據存儲
│       └── tracking.db               # SQLite數據庫
│
├── templates/                         # 主應用模板
├── static/                            # 主應用靜態文件
└── ... 其他文件夾 ...
...
#### **`tracking.js`** - 統一腳本（300-500行）
```javascript
/**
 * 所有頁面的交互邏輯
 * 包括：Tab切換、篩選、快速更新、API調用等
 */
```
...
# 實際路徑：http://localhost:5000/tracking
```
...
# 訪問：http://localhost:8000/tracking
```
...
#### **網頁路由（給人用）**
```
/tracking/                          # 主頁（重定向到登入或儀表板）
/tracking/login                     # 登入頁
/tracking/logout                    # 登出
/tracking/index                     # 儀表板
/tracking/orders                    # 訂單列表
/tracking/orders/<number>           # 訂單詳情
/tracking/orders/new                # 新增訂單
/tracking/orders/<number>/edit      # 編輯訂單
/tracking/revisions                 # 修圖列表
/tracking/revisions/<number>        # 修圖詳情
/tracking/reports                   # 統計報表
```

#### **API路由（給APP用）**
```
/tracking/api/auth/login            # 登入API
/tracking/api/auth/logout           # 登出API
/tracking/api/auth/me               # 當前用戶
/tracking/api/orders                # 訂單列表
/tracking/api/orders/<number>       # 訂單詳情
/tracking/api/orders/quick-update   # 快速更新
/tracking/api/revisions             # 修圖列表
/tracking/api/stats                 # 統計數據
```
